...
**Goal:** Promote closure-receipt emission from a per-plan pattern to a runtime feature. Every closed plan leaves an audit-grade receipt on disk automatically via a hook-based close-transition trigger, and a `plan-enforcer-receipt` CLI plus `/plan-enforcer-receipt` skill let users emit receipts on demand. Every receipt is preserved as its own timestamped file, and every receipt links to its prior closure for the same plan-slug, turning the directory of receipts into a walkable audit chain.

**Constraints:**
- Pure Node, no new runtime dependencies. Uses only system Node and existing helpers in `src/` (ledger-parser, archive, etc.).
- ASCII-only in every committed file -- receipt bodies, CLI stdout, skill prose, test fixtures. No Unicode box-drawing, no em-dashes, no smart quotes.
- Brand-protection forbidden-language grep (CLAUDE.md / global) must pass on every new committed file.
- Test runner must be `node --test` to match existing `tests/*.test.js` suite. Test files under `tests/` use that runner.
- Auto-emission must work at `structural` tier (this project's default; that is the tier in use right now). `enforced` tier must also continue to work.
- Auto-emission must NOT fire at `advisory` tier (no hooks are wired there -- skill-only mode).
- Receipts land in `.plan-enforcer/proof/closure-<plan-slug>-<utc-iso>.md` -- filename-safe UTC ISO (colons replaced with hyphens).
- Every receipt MUST include a "Prior closure" section that either links to the immediately-prior receipt for this plan-slug or states "none (first close of this plan)". Chain is non-negotiable.
- Every emission must be its own file. Never overwrite an existing closure file; never coalesce closures for the same plan into one.
- Hook failure degrades gracefully: if `git` is absent, if the ledger is malformed, if write permissions fail, the hook logs and exits non-blocking. Receipt emission must never block a ledger edit.
- Existing `hooks/session-end.js` semantics stay unchanged. The new emission is separate logic.
- Code style matches existing `src/*-cli.js` patterns (shebang + requires + functions + `main()` at bottom; module.exports for testable parts).

**Out of scope:**
- Per-plan receipt customization beyond existing Proof Requirements in discuss packets.
- Receipt templating language, theme system, or user-configurable sections.
- HTML, PDF, or any non-markdown receipt format.
- Notifications, webhooks, Slack / email integration, auto-merge, auto-PR creation on close.
- Retroactive receipt generation for historical closed plans (the feature applies to plans that close after install).
- README-rebuild T30 verification (blocked on a separate workstream).
- Any change to `docs/playground/readme-playground.html`, `scripts/readme-playground-server.js`, or `tests/readme-playground.test.js` (user's uncommitted WIP).
- Any fix to the pre-existing `tests/statusline-hook.test.js` failures (covered by D2 override from the prior plan).
- Rewriting the existing `plan-enforcer-report` or `plan-enforcer-status` skills to use the new receipt system (they remain live-view surfaces; receipts are an emission-event surface).
...
- The project's current `structural` tier is the target tier for the auto-emission-on-close feature. `enforced` tier will continue to work; `advisory` tier has no hooks wired, so auto-emission does not apply there (users at advisory tier still get the explicit CLI / skill).
- The post-edit ledger is readable at the time the PostToolUse hook fires. Claude Code's documented hook semantics indicate PostToolUse fires after the edit has been written to disk, so `fs.readFileSync('.plan-enforcer/ledger.md', 'utf8')` returns the post-edit content.
- `toolContext.tool_input.old_string` is available on Edit-tool events (matches post-tool.js reading it for other purposes). If it is absent on non-Edit events (e.g., Write), the hook falls back to the `.last-close-hash` sidecar for idempotence.
- `git diff --stat` output format is stable enough to include verbatim in the receipt without further parsing. Where format changes over git versions, the receipt simply includes the raw stdout.
- Pre-existing `tests/statusline-hook.test.js` failures (D2 override) remain out of scope; new tests in Task 6 do NOT depend on that file.
- `docs/playground/readme-playground.html` and `scripts/readme-playground-server.js` remain uncommitted (user's in-flight workstream); this plan does not touch them.
- The ledger's `<!-- source: docs/plans/<file>.md -->` metadata accurately points at the source plan for every closed plan. If missing, slug derivation falls back to `ledger-<iso>` with a note in the receipt.
