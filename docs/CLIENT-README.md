...
Your AI agent said it shipped the feature. It did not.

Every team running Claude Code, Cursor, or any long-running agent has seen one or more of these fail states:

- **Silent narrowing.** You asked for "clean up auth flow." The model quietly shipped a rename and declared victory. The contract you actually signed up for is now lost, and nobody can tell you when it slipped.
...
- **Unrecoverable resume.** The session died. The next session read a stale plan and rebuilt work that was already done, or skipped work that wasn't.
- **Cold-review failure.** Someone else (regulator, senior, auditor) opens the repo six months later and cannot reconstruct why any given change exists.
...
**Translation:** when the contract changes mid-build, the other two systems still ship *a* feature. They do not ship *your* repaired feature. Plan Enforcer does, under repeated mutation and interruption, 172 times in a row.

Source: `benchmarks/framework-comparison/ASK-FIDELITY-*-COMPARISON.md` (one markdown per scenario, each backed by `outcome.json` + `ask-fidelity.json`).
...
Scenario B is a planning-only trap that forces a real interpretation fork between safe replay and a weaker retry framing.
...
Only Plan Enforcer's `combobulate → draft → review` chain preserves the full semantic defense of the original ask. The other two draft good-looking plans that have already dropped a constraint.
...
Source: `benchmarks/framework-comparison/PHASED-SMALL-COMPARISON.md`.
...
- **Combobulate** — turns a fuzzy ask ("clean up auth flow") into an intent-defense packet with verbatim user quotes, non-negotiables, and forbidden narrowings. This is the anchor every downstream stage is measured against.
- **Draft** — writes the concrete executable plan into `docs/plans/<slug>.md`: repo-aware, proof-aware, phased. Knows your file layout, your test patterns, your config.
...
- **Bounded phases.** Large work is cut into phases. Each phase has its own verifier output. No more 98-task single-session collapse.
- **Single-file ledger.** `.plan-enforcer/ledger.md` holds every task, status, evidence pointer, and chain-of-custody reference on disk. Human-readable, machine-parseable, git-diffable.
- **Tier dial.** Three intensity levels you choose:
  - `advisory` — logs everything, blocks nothing. Use while learning.
  - `structural` — warns on drift, blocks hard breaks. **Default.**
  - `enforced` — blocks every unplanned edit until logged. CI-safe.
- **Crash-proof.** Session dies? `SessionStart` hook reads the ledger on next launch, knows which tasks are verified and which are pending, and resumes at the right row. No re-planning, no re-doing finished work.
...
- **Lineage + supersession + dossier.** When a contract gets repaired mid-build, the chain captures the repair — not just the final state.
- **Final-truth closure.** Cross-phase sign-off with attestation. The thing you sign off on is what actually landed, not what the agent claimed landed.

---

## Capability matrix

Every `Yes` below is backed by a concrete file reference in `docs/strategy/capability-matrix.md`. No marketing-only claims.
...
- Platform teams running fleets of Claude Code / Cursor sessions at scale.
...
Installs as Claude Code skills + hooks. Works with any project. Default tier is `structural`. Flip to `advisory` to learn, `enforced` for CI.

```bash
plan-enforcer-config              # show current tier + settings
plan-enforcer-config --tier enforced
plan-enforcer-status              # scoreboard + current task + unverified items
plan-enforcer-chain T5            # custody for task T5
...
That is it. The `SessionStart` hook auto-detects the plan, generates the ledger, and injects the enforcement protocol. Every subsequent edit is traced.

**No plan file yet?** Draft one from a goal inside Claude Code:

```
Use plan-enforcer-draft to create a plan for "Add rate limiting to the login endpoint"
```
...
If you already use GSD or Superpowers, point Plan Enforcer at an existing plan:

```
Use plan-enforcer-draft to import docs/plans/existing-gsd-plan.md
```

The enforcement layer runs regardless of who drafted the plan.
...
- [`COST-COMPARISON.md`](benchmarks/framework-comparison/COST-COMPARISON.md) — per-cell token + dollar breakdown
...
- [`PHASED-SMALL-COMPARISON.md`](benchmarks/framework-comparison/PHASED-SMALL-COMPARISON.md) — 24-task parity read
- [`PHASED-LARGE-TRUST-COMPARISON.md`](benchmarks/framework-comparison/PHASED-LARGE-TRUST-COMPARISON.md) — 98-task trust pack
...
At `advisory` tier, it is pure logging. At `structural` (default), the only blocks are on edits that have no plan row and no D-row — which is the thing you *want* blocked. At `enforced`, CI-grade blocking. The ~25% wall-clock advantage on 98-task trust (vs the nearest competitor) shows it does not cost you speed on real work.
...
**Benchmarks:** `benchmarks/framework-comparison/` in this repo.  
**Technical deep dive:** [`README.md`](README.md) — engineering-facing doc.  
**Strategy context:** `docs/strategy/`.
